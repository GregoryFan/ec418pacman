...
from train import to_gray, make_initial_stack, update_stack 
...
# ───────────────────────── helper ─────────────────────────
STACK_SIZE = 4  # must match train.py
...
def play_visual(layout: str, net: DQN, episodes: int, delay_ms: int, scale: int):
    ...
    for ep in range(1, episodes + 1):
        ...
        state = make_initial_stack(frame)   # (H, W, 4)
        done, step, win = False, 0, False

        while not done and step < 1000:
            with torch.no_grad():
                q_values = net(torch.as_tensor(state, device=DEVICE))
            ...
            # next_state = push_frame(state, next_frame)
...
if __name__ == "__main__":
    ...
    state_rgb, _ = tmp_env.reset()
    frame = to_gray(state_rgb)
    stack = make_initial_stack(frame)   # (H, W, STACK_SIZE)
